...
export function DiscoverCard({ item, onClick }: DiscoverCardProps) {
    return (
        ...
        >
            {/* Image Placeholder - Since real images aren't provided, use a colored div with grain */}
            <div className={cn(
                "absolute inset-0 transition-transform duration-700 group-hover:scale-105",
                // Random muted gradients for placeholders
                item.id % 4 === 0 ? "bg-gradient-to-br from-blue-900 to-indigo-900" :
                    item.id % 4 === 1 ? "bg-gradient-to-br from-cyan-900 to-teal-900" :
                        item.id % 4 === 2 ? "bg-gradient-to-br from-slate-900 to-zinc-900" :
